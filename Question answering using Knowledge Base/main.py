...
def train(cfg):
    ...
    my_model = get_model(cfg, train_data.num_kb_relation, len(entity2id), len(word2id)).to(device)
...
def inference(my_model, valid_data, entity2id, cfg, log_info=False):
    ...
    for iteration in tqdm(range(valid_data.num_data // test_batch_size)):
        ...
        eval_loss.append(float(loss.data))
        eval_acc.append(acc)
...
def test(cfg):
    ...
    my_model = get_model(cfg, test_data.num_kb_relation, len(entity2id), len(word2id)).to(device)
    test_acc = inference(my_model, test_data, entity2id, cfg, log_info=True)
    return test_acc
